	...
	if($_POST['order_id'] == '') { // Add
		...
		if(db_query($q,"Insert Order")) {
				...
				if(isset($_POST['product_ids_'.$row['id']]) && count($_POST['product_ids_'.$row['id']])>0) { // Start the insert of products
					foreach($_POST['product_ids_'.$row['id']] AS $k => $id) {
						if(isset($_POST['qty_'.$row['id'].'_'.$id]) && is_numeric($_POST['qty_'.$row['id'].'_'.$id]) && $_POST['qty_'.$row['id'].'_'.$id] > 0) {
							$q2 .= "('".($_POST['approved_'.$row['id'].'_'.$id]=='on'?'1':'0')."','".$id."','".$info['id']."','".$row['id']."','".$newID."','".$_POST['qty_'.$row['id'].'_'.$id]."','".$_SESSION['user_id']."','".$_POST['price_'.$row['id'].'_'.$id]."','".$_POST['setup_'.$row['id'].'_'.$id]."',NOW(),NOW(),'".(!in_array($id,$urproducts)?'1':'0')."'),";
						}
					}
				}
				if($q2 != "") {
					db_query("INSERT INTO session_order_items (approved,product_id,session_id,timeslot_id,order_id,quantity,user_id,price,setup,created_at,updated_at,added_by_admin) 
			...
			db_query("UPDATE session_orders SET 
						...
						contingency='".($info['bill_other']?$_POST['contingency']:'0')."',
						revision='1',
						updated_at=now()
						WHERE id='".$newID."'
			","Update Order");
			
			//Now lets do the order notes and presenter brought items
			$q2 = "";
			...
			$_SESSION['infoMessages'][] = "Order Saved. Order Number: ".date('y')."TSMNECC-".$newID."-1";
		} else {
		...
		while($row = mysqli_fetch_assoc($datestimes)) {
			...
			db_query("UPDATE session_order_items SET 
						...
						WHERE !deleted AND timeslot_id='".$row['id']."' AND order_id='".$orderinfo['id']."'".($temp_all_product_ids != ""?" AND product_id NOT IN (".$temp_all_product_ids.")":""),"Remove products unchecked");
						
			$existing_products = array();
			if($temp_all_product_ids != "") {
				$grab_ids = db_query("SELECT * FROM session_order_items 
				WHERE timeslot_id='".$row['id']."' AND order_id='".$orderinfo['id']."' 
					AND product_id IN (".$temp_all_product_ids.")","Get all info");
			...
			if(isset($_POST['product_ids_'.$row['id']])) {
				foreach($_POST['product_ids_'.$row['id']] AS $k => $id) {
					$_POST['approved_'.$row['id'].'_'.$id] = (isset($_POST['approved_'.$row['id'].'_'.$id]) ? '1' : '0'); 
					if(isset($existing_products[$id]) && ($_POST['qty_'.$row['id'].'_'.$id] > 0 && is_numeric($_POST['qty_'.$row['id'].'_'.$id])) && ($existing_products[$id]['quantity'] != $_POST['qty_'.$row['id'].'_'.$id] || (isset($_POST['price_'.$row['id'].'_'.$id]) && $existing_products[$id]['price'] != $_POST['price_'.$row['id'].'_'.$id]) || (isset($_POST['setup_'.$row['id'].'_'.$id]) && $existing_products[$id]['setup'] != $_POST['setup_'.$row['id'].'_'.$id]) || $existing_products[$id]['approved'] != $_POST['approved_'.$row['id'].'_'.$id])) { //update
							
						db_query("UPDATE session_order_items SET 
									quantity='".$_POST['qty_'.$row['id'].'_'.$id]."',
									approved='".$_POST['approved_'.$row['id'].'_'.$id]."',
									price='".(isset($_POST['price_'.$row['id'].'_'.$id])?$_POST['price_'.$row['id'].'_'.$id]:'0')."',
									setup='".(isset($_POST['setup_'.$row['id'].'_'.$id])?$_POST['setup_'.$row['id'].'_'.$id]:'0')."',
									updated_at=now(),
									deleted=0
									WHERE id='".$existing_products[$id]['id']."'
						","Update ".$existing_products[$id]['id']);

					} else if(!isset($existing_products[$id]) && $_POST['qty_'.$row['id'].'_'.$id] > 0 
						&& is_numeric($_POST['qty_'.$row['id'].'_'.$id])) { //insert
						
						db_query("INSERT INTO session_order_items SET 
									approved='".(isset($_POST['approved_'.$row['id'].'_'.$id])?'1':'0')."',
									price='".(isset($_POST['price_'.$row['id'].'_'.$id])?$_POST['price_'.$row['id'].'_'.$id]:'0')."',
									setup='".(isset($_POST['setup_'.$row['id'].'_'.$id])?$_POST['setup_'.$row['id'].'_'.$id]:'0')."',
									product_id = '".$id."',
									session_id = '".$info['id']."',
		...
		db_query("UPDATE session_orders SET 
					status_id = '".$_POST['status_id']."',
					".(isset($_POST['contingency'])?" contingency='".$_POST['contingency']."',":"")."
					updated_at = NOW(),
					revision = '".($orderinfo['revision']+1)."'
					WHERE id='".$orderinfo['id']."'
		
		","Update Order");

		$_SESSION['infoMessages'][] = "Order Saved. Order Number: ".$orderinfo['quote_name']."-".($orderinfo['revision']+1);
	}
