<?
	...
	$breadcrumbs[] = array('URL' => $BF.'admin/sessiontypes/','TEXT'=>"Session Types");
	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			$q = "SELECT sessiontypes.*
					FROM sessiontypes
					WHERE !sessiontypes.deleted
					ORDER BY sessiontypes.sessiontype_name";
				
			$results = db_query($q,"getting Session Types");
			# Stuff In The Header
			function sith() { 
				global $BF;
			...
			$page_info['instructions'] = "Click a Session Type to edit or add a new Session Type.";
		...
		case 'add.php':
			...
			if(isset($_POST['sessiontype_name'])) { include($post_file); }
...
<?
			...
			$page_info['title'] = "Edit ".$section.": ".$info['sessiontype_name'];
