...
function pretty_datetime2($datetime) {
...
function auth_check($sitm='',$access="",$show_needed="1") {
	...
	if(!isset($_SESSION['user_id'])) {  // if this variable is set, they are already authenticated in this session
		if(isset($_COOKIE['user_id']) && is_numeric($_COOKIE['user_id'])) {
			...
			WHERE !users.deleted AND users.id='" . $_COOKIE['user_id'] . "'
			";
			$result = db_query($q, "Logging Back In.");
		...
		if ($_SESSION['logedin_at'] <= date('m/d/Y H:i:s',strtotime("-12 hours"))) {
			header("Location: ".$BF."index.php?logout=3");
			die();
		} 
