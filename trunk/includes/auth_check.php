<?
	...
	if($br->Name == 'MSIE' && $br->Version < 7) {
		errorPage('Sorry, Internet Explorer version '.$br->Version.', is not compatible with this site. Please update to Version 7 or higher.');
	}
	...
	if (isset($_POST['auth_form_name'])) {  // check to see if this is a submission of the login form
		...
		if (mysqli_num_rows($result)) {
			...
			if($pass == $row['crypted_password'] && $row['enabled']) {
				...
				setcookie("user_id", $row['id'], time()+60*60*24, '/');  /* expire in 2 days */
